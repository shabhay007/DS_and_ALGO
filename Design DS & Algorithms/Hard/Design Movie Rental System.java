...
// Approach 1 (Brute Force) - Using ArrayList + custom sorting
// T.C. - O(klog(k)); k = entries.length - in each search and report call which is very expensive
// S.C. - O(k)
class MovieRentingSystem {
    ...
    public MovieRentingSystem(int n, int[][] entries) {
        ...
        // adding the movies into movies list
        for(int[] entry : entries){
            int shop = entry[0];
            int movie = entry[1];
            int price = entry[2];

            movies.add(new int[]{shop, movie, price});
        }
    }
    ...
    public List<Integer> search(int movie) {
        ...
        for(int i = 0; i<movies.size(); i++){
            int shop = movies.get(i)[0];
            int movieName = movies.get(i)[1];
    ...
    public void rent(int shop, int movie) {
        // adding to rented list
        for(int i = 0; i<movies.size(); i++){
            int s = movies.get(i)[0];
            int m = movies.get(i)[1];
            int price = movies.get(i)[2];
    ...
    public void drop(int shop, int movie) {
        // adding back to movies list after return
        for(int i = 0; i<rented.size(); i++){
            int s = rented.get(i)[0];
            int m = rented.get(i)[1];
            int price = rented.get(i)[2];
    ...
    public List<List<Integer>> report() {
        Collections.sort(rented, new Comparator<int[]>(){
            public int compare(int[] a, int[] b){
                if(a[2] != b[2]){
                    return a[2] - b[2];
                }
        ...
        for(int i = 0; i<rented.size(); i++){
            int shop = rented.get(i)[0];
            int movie = rented.get(i)[1];

            response.add(Arrays.asList(shop, movie));
            count++;
